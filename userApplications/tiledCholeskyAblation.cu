...
int main(int argc, char *argv[]) {
  ...
  if (cmdl["-h"] || cmdl["--help"]) {
    ...
    fmt::print("Options:\n");
    fmt::print("  --N=<size>              Matrix dimension (default: 1024)\n");
  ...
  // Get matrix parameters using named arguments
  cmdl("--N", 1024) >> N;  // Matrix dimension
  cmdl("--T", 4) >> T;     // Number of tiles
